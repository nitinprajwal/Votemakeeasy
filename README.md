...
See [here][users] for a list of events that have used Voting System in the past.

**If you use Voting System for your event, please add yourself to the list! It only
takes a minute, and knowing that Voting System is helping real events helps keep us
going <3**
...
Voting System is based on the method of pairwise comparisons. Before you use Voting System,
...
Voting System is stable software. We've used it successfully at HackMIT, and a bunch of
other hackathons and events have used it too.

Voting System is a pretty different way of doing judging. If you want to use this for
...
If you're able to contribute to making Voting System better, that would be **awesome**!
...
Voting System` (unless you're using a different database name). Before you use the app,
you need to initialize the database by running `python initialize.py`. **Note
that Voting System does not preserve database schema compatibility between versions.**
...
Voting System:app`. This is sufficient if you want to serve Voting System under its own domain
(e.g. `judging.example.com`). If you are serving Voting System under a subpath, e.g.
...
Voting System behind a proxy server, be sure to set `PROXY=true` in Voting System's settings.

For sending emails, you'll also need to start a celery worker with `celery -A
Voting System:celery worker`.
